...
class Blockchain {
  ...
  submitStar(address, message, signature, star) {
    let self = this;
    return new Promise(async (resolve, reject) => {
      let submissionTime = parseInt(message.split(":")[1]);
      let currentTime = parseInt(new Date().getTime().toString().slice(0, -3));
      let elapsedTime = currentTime - submissionTime;
      if (elapsedTime / 60000 < 5) {
        ...
        reject(new Error("Too much time elapsed."));
      }
    });
  }
  ...
  validateChain() {
    ...
    return new Promise(async (resolve, reject) => {
      self.chain.forEach(async (block) => {
        ...
        if (!isValid) {
          errorLog.push("Problem validating block ${block.height}.");
          return;
        }

        let prevBlock = await self.getBlockByHeight(block.height - 1);
        if (block.previousBlockHash != prevBlock.hash) {
          errorLog.push("Problem with previous hash of block ${block.height}.");
        }
      });
      resolve(errorLog);
    });
